...
public class Authentification extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        emailOrNickname = new String(request.getParameter("emailOrNickname").getBytes("ISO-8859-1"),"UTF8");
        request.setAttribute("emailOrNickname", emailOrNickname);
        pwd = new String(request.getParameter("password").getBytes("ISO-8859-1"),"UTF8");
        
        user = this.accountFacade.findByEmail(emailOrNickname);
        if (user == null){
            user = this.accountFacade.findByNickname(emailOrNickname);
        }
